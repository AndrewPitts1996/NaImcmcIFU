...
# Read in DAP file
mcmcdir = '/data/home/krubin/Projects/MaNGA/NaImcmc/Analysis/MCMC/SQUARE2.0/FluxNoSModFit/'
...
for nn in range(len(allmcmc)):
    ...
    fout = mcmcdir+plate_str+"-"+ifu_str+"-fit-map-N-EW.pdf"
    ...
    if(((os.path.exists(fout)) & (overwrite==False)) | (plate_str+"-"+ifu_str=='9501-12705')):
        ...
        with fits.open(fits_NaI_map) as hdu_NaImaps:
            ...
            NaIEW_map = hdu_NaImaps['EWNAI_OBS_MAP'].data
            NaIlogN05_map = hdu_NaImaps['NAILOGN05_MAP'].data
            NaICf50_map = hdu_NaImaps['NAICF50_MAP'].data
            
            NaIvel50_msk = np.zeros(binid.shape,dtype=bool)
            NaIvel50_sigflowmsk = np.ones(binid.shape,dtype=bool)
        
            bdbin = np.where((NaIvel84_map - NaIvel16_map) > 70.0)
            NaIvel50_msk[bdbin] = True
                
            ind_outflow = np.where((NaIvel95_map < 0.0) & (NaIelvel95_map < 0.0) &
                                   (np.fabs(NaIvel50_map) > vflow_min) &
                                   ((NaIvel84_map - NaIvel16_map) < 70.0))

            ind_inflow = np.where((NaIvel05_map > 0.0) & (NaIelvel05_map > 0.0) &
                                  (np.fabs(NaIvel50_map) > vflow_min) &
                                  ((NaIvel84_map - NaIvel16_map) < 70.0))
            ...
            NaIlogN05_mapmsk = np.ma.MaskedArray(NaIlogN05_map, mask=NaIvel50_msk)
            ...
            mapplot.plot(value=NaIvel50_mapmsk, mask=NaIvel50_msk, title='NaI Absorption Velocity',
                         cblabel='Relative Velocity (km/s)', cmap=cm.coolwarm, cbrange=[-80,80], fig=fig, ax=axes[1])
            mapplot.plot(value=NaIvel50_sigflow, mask=NaIvel50_sigflowmsk, title='Significant In/Outflow',
                         cblabel='Relative Velocity (km/s)', cmap=cm.coolwarm, cbrange=[-80,80], fig=fig, ax=axes[2])
            mapplot.plot(value=NaIlogN05_mapmsk, mask=NaIvel50_msk, title='log N(NaI)',
                         cblabel='log N(NaI) [$cm^{-2}$]', cmap=cm.viridis, cbrange=[9,13], fig=fig, ax=axes[3])
            mapplot.plot(value=NaIEW_map, title='Total Equivalent Width',
                         cblabel='(Angstroms)', cmap=cm.coolwarm, cbrange=[-3,3], fig=fig, ax=axes[4])
