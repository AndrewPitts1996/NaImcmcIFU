...
def fig_mcmc_triangle(mcmcdirroot=None, overwrite=False, FIT_FLG=None,
                      plate_plot=None, ifu_plot=None):
        ...
        if(((os.path.exists(outfil)) & (overwrite==False)) | (plate_str+"-"+ifu_str=='9501-12705') | (plate_str+"-"+ifu_str=='8719-9101') | (plate_str+"-"+ifu_str=='8940-9101')):
            ...
            with PdfPages(outfil) as pdf:
                ...
                for qq in range(len(tmcmc['bin'])):
                    ...
                    ax.plot(restwave_NaI,cont_NaI, color='purple')
                    ...
                    ax.text(0.4, 0.95, vel_str, ha='left', va='center', transform=ax.transAxes)
                    elvel_str = "{:.1f}".format(elvel05[ind_map])+","+"{:.1f}".format(elvel50[ind_map])+\
                        ","+"{:.1f}".format(elvel95[ind_map])
                    ax.text(0.4, 0.89, elvel_str, ha='left', va='center', transform=ax.transAxes)
                    ...
                    if((vel95[ind_map] < 0.0) & (elvel95[ind_map] < 0.0) &
                       ...
                       ((vel84[ind_map]-vel16[ind_map])<200.0)):
                        ax.text(0.1,0.1,'OUTFLOW',ha='left',va='center',
                                transform = ax.transAxes,color="blue")
                    
                    if((vel05[ind_map] > 0.0) & (elvel05[ind_map] > 0.0) &
                       (np.fabs(vel50[ind_map]) > 40.0) &
                       ((vel84[ind_map]-vel16[ind_map])<200.0)):
                        ax.text(0.1,0.1,'INFLOW',ha='left',va='center',
...
def main():
    ...
    # Using bins selected for fake flow sims
    ...
    for qq in range(len(fakebins)):
        ...
        fig_mcmc_triangle(mcmcdirroot=None, overwrite=True, FIT_FLG=0, plate_plot=int(fakebins['PLATE'][qq]), \
                          ifu_plot=int(fakebins['IFU'][qq]))
