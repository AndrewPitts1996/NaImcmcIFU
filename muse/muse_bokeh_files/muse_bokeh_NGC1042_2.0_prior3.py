...
stellar_vel =  NGC1042_map['STELLAR_VEL'].data
stellar_sigma = NGC1042_map['STELLAR_SIGMA'].data
binid_map = NGC1042_map['BINID'].data[0]

# emission line header dictionary
emline = {}
for k, v in NGC1042_map['EMLINE_GFLUX'].header.items():
    if k[0] == 'C':
        try:
            i = int(k[1:])-1
        except ValueError:
...
halpha_map = NGC1042_map['EMLINE_GFLUX'].data[emline['Ha-6564'],:,:]
# H-beta emission line flux from MUSE cube
hbeta_map = NGC1042_map['EMLINE_GFLUX'].data[emline['Hb-4862'],:,:]
...
# recessional velocity of NGC 1042 (SIMBAD value) (km/s)
...
# dc = cube.DataCube(inp=NGC1042_cube_file)
# n_band = transform.narrowband(dc, 5880, 5940, mode='median')
# img_data = n_band.data

# obtain flux array from MUSE cube
flux = NGC1042_cube['FLUX'].data
model = NGC1042_cube['MODEL'].data
ivar = NGC1042_cube['IVAR'].data
error = np.sqrt(1/ivar)
wave = NGC1042_cube['WAVE'].data
...
# directory path
path1 = '/Users/erickaguirre/Desktop/NaI_MCMC_output/NGC1042_2.0_test_prior3/'
# fits file path
fits_paths = sorted(glob.glob(path1+'NGC1042*'), key = sort_key)
...
redshift = 0.00460  # NGC 1042 redshift's
...
p8_1 = figure(title="Samples logN prior2 range (12.0-20.0)", y_axis_label='lambda_red')
...
# Plasma265 (purple to orange)
# Blues8 (blue to white)
color_mapper9 = LogColorMapper(palette='Inferno256',low=0.01,high=1.5*np.max(halpha_map_corrected))
...
layout_row1 = row(children=[p5,p9,p10])
