...
paths = '/Users/erickaguirre/Desktop/NaI_MCMC_output/NGC4030_2.0_err_corr/'
...
source2 = ColumnDataSource( data = dict( ew_map=[ew_D2_map],
                                       bin_id_map=[binid_map] ) )
gas_vel_map[(ew_D2_map<0) | (binid_map == -1)] = np.nan
# ColumnDataSource for gas velocity map
source5 = ColumnDataSource( data = dict( gas_vel=[gas_vel_map],
                                       bin_id_map=[binid_map] ) )
...
tooltips2 = [("Spaxel (x,y)", "($x{0}, $y{0})"), ("EW", "@ew_map{0.00}"),("Bin ID","@bin_id_map")]
...
p2 = figure(title='Na I EW Map',tools=tools1,width=535, height=480,toolbar_location="below")
# p3 = figure(title='Stellar Dispersion Map',tools=tools1,width=535, height=480,toolbar_location="below")
...
# plot 2 Na I equivalent width map
color_mapper2 = LinearColorMapper(palette=list(reversed(Turbo256)),low=-0.6,high=0.6)
color_bar2 = ColorBar(color_mapper=color_mapper2, label_standoff=12,title='EW (angstroms)',
                      major_label_text_align = 'right',
                      title_standoff = 4,title_text_align='right',title_text_font_size = '15px')
equiv_width = p2.image(image='ew_map', source=source2,x=0,y=0,
                            dw=stellar_vel.shape[0],dh=stellar_vel.shape[1],
...
# box 1: move the box along with the Mouse event
# box 2: move the box along with the Tap event
equiv_width_box1 = Scatter(x = 195 ,y = 195 ,size = 10, fill_alpha = 0.5,marker="square")
equiv_width_box2 = Scatter(x = 195 ,y = 195 ,size = 10, fill_alpha = 0.5,marker="square")
...
NaI_gas_mod = p6.line('wave','gas_model_flux',source=source6,color='purple',legend_label='Gas Model')
...
layout_row2 = row(children=[p6,p2])
