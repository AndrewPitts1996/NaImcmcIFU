...
# # NGC1042_output_dir = 'output2.0_NGC1042/'
# # NGC1042_cube_dir = 'SQUARE2.0-MILESHC-MASTARSSP/1/2/'

# log cube
# NGC1042_cube_file = main_dir3+ NGC1042_output_dir+ NGC1042_cube_dir+'manga-1-2-LOGCUBE-SQUARE2.0-MILESHC-MASTARSSP.fits'
# NGC1042_cube = fits.open(NGC1042_cube_file)
# # log maps
# NGC1042_map_file = main_dir3+ NGC1042_output_dir+ NGC1042_cube_dir+'manga-1-2-MAPS-SQUARE2.0-MILESHC-MASTARSSP.fits.gz'
# NGC1042_map = fits.open(NGC1042_map_file)
...
# # NGC 4030 
NGC4030_output_dir = 'output1.0_NGC4030/'
NGC4030_cube_dir = 'SQUARE1.0-MILESHC-MASTARSSP/1/1/'
# log cube
NGC4030_cube0_6_file = main_dir3+ NGC4030_output_dir+ NGC4030_cube_dir+'manga-1-1-LOGCUBE-SQUARE1.0-MILESHC-MASTARSSP.fits'
NGC4030_cube0_6 = fits.open(NGC4030_cube0_6_file)
# log maps
NGC4030_map_file = main_dir3+ NGC4030_output_dir+ NGC4030_cube_dir+'manga-1-1-MAPS-SQUARE1.0-MILESHC-MASTARSSP.fits.gz'
...
halpha_map_corrected = halpha_map * (10**(0.4*3.33*ebv_map))
...
r = (29.90 * 1e6 * u.parsec).to(u.centimeter)
...
# dc = cube.DataCube(inp=NGC1042_cube)
...
# directory path
path1 = '/Users/erickaguirre/Desktop/NaI_MCMC_output/NGC4030_1.0_test_prior3/'
# fits file path
...
tooltips11 = [("(x,y)", "($x{0}, $y{0})"), ("E(B-V) (mag)", "@image{0.00}")]
...
p6 = figure(title='DAP 1.0 Na I Region w/ MCMC Best-Fit Model',tools=tools1,plot_width=600,plot_height=480,
            toolbar_location="left")

p8_1 = figure(title="Samples logN prio3 range (12.0-20.0)", y_axis_label='lambda_red')
...
p11 = figure(title='E(B-V) Map',tools=tools1, plot_width=535, plot_height=480,toolbar_location="below")
...
# Plasma265 (purple to orange)
# Blues8 (blue to white)
color_mapper9 = LogColorMapper(palette='Inferno256',low=np.min(halpha_map[halpha_map>-999]),
                               high=np.max(halpha_map[halpha_map>-999]))
...
halpha_image = p9.image(image=[np.transpose(halpha_map[halpha_map>-999])], x=0, y=0,
                        dw=halpha_map.shape[0],dh=halpha_map.shape[1], 
...
# low=0.2*np.min(gas_vel_map), high=0.2*np.max(gas_vel_map)
color_mapper10 = LinearColorMapper(palette='Inferno256',low=-7,high=-4.4)
...
# plot 11 MUSE E(B-V) dust attentuation map plot
# low=0.2*np.min(gas_vel_map), high=0.2*np.max(gas_vel_map)
color_mapper11 = LinearColorMapper(palette=list(reversed(cc.fire)),low=np.min(ebv_map[ebv_map>-999]),
                                   high=np.max(ebv_map[ebv_map>-999]))
color_bar11 = ColorBar(color_mapper= color_mapper11, label_standoff=12)
#color_bar10 = ColorBar(color_mapper=color_mapper10, label_standoff=12,title='Solar Mass/year',title_text_align = 'center')
ebv = p11.image(image=[np.transpose(ebv_map)], x=0, y=0, dw=ebv_map.shape[0],dh=ebv_map.shape[1],
         color_mapper=color_mapper11)
p11.add_layout(color_bar11,'right')

# add hover tool
p11.add_tools(HoverTool(tooltips = tooltips11, renderers = [ebv]))

# create SFR image box highlight in plot 10
# box 1: move the box along with the Mouse event
# box 2: move the box along with the Tap event
ebv_box1 = Square(x = 195 ,y = 195 ,size = 10, fill_alpha = 0.5)
ebv_box2 = Square(x = 195 ,y = 195 ,size = 10, fill_alpha = 0.5)
p11.add_glyph(ebv_box1)
p11.add_glyph(ebv_box2)
...
def update_data(attr):
    ...
    p8_1_update = figure(title="Samples logN prior1 range (11.0-16.1)", y_axis_label='lambda_red')
    ...
    # update SFR box highlight in plot 11
    ebv_box1.update(x=x,y=y)
...
def update_box(attr):
    ...
    if x > stellar_vel.shape[0]:
        ...
        # update h-alpha velocity image box highlight in plot 9
        ...
        # update e(b-v) image box highlight in plot 11
        ebv_box2.update(x=stellar_vel.shape[0] ,y=y)
        ...
        ebv_box2.update(x=x ,y=stellar_vel.shape[1])
...
p11.on_event(events.Tap,update_data)
p11.on_event(events.MouseMove,update_box)

layout_row1 = row(children=[p5,p9,p11])
curdoc().add_root(layout_row1)
# layout_row2 = row(children=[p6,g,grid8])
# layout_row2 = row(children=[p6,g])
# curdoc().add_root(layout_row2)
# layout_row3 =row(children=[p11])
