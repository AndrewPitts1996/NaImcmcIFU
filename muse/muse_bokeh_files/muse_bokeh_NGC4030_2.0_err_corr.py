...
# NGC 4030
NGC4030_output_dir = 'output2.0_NGC4030_NOISM_err_corr/'
...
file = fits.open('NGC4030_2.0_err_corr_mcmc_output.fits')
...
def get_vel_width_map(binid_map, table_uniq):
    ...
    # create velocity width map
    vel_widths = table_uniq['upper_vel_sig'] + table_uniq['lower_vel_sig']
    vel_width_map = np.zeros(binid_map.shape)

    for binid in table_uniq['bin']:
        vel_width = vel_widths[table_uniq['bin']==binid]
        ind = binid_map == binid
...
def get_text_labels(table_uniq,bin_indx):
    ...
    gas_vel_sig = table_uniq['vel_sigma_mean'][bin_indx][0]
    ...
    return gas_vel_text, text_logN, text_bd, text_Cf # text_lamred
# -----------------------------------------------------------------
...
x,y = 195,195
binid_label = Label(x=5,y=31, x_units='screen', y_units='screen', text = binid_text)
spaxel_text_p6 = 'Spaxel: (' +str(x) +','+str(y)+')'
spaxel_label_p6 = Label(x=5,y=16, x_units='screen', y_units='screen', text = spaxel_text_p6)
# text labels
gas_vel_text, text_logN, text_bd, text_Cf = get_text_labels(table_uniq,bin_indx)

gas_vel_label_p6 = Label(x=5,y=1, x_units='screen', y_units='screen', text = gas_vel_text)
