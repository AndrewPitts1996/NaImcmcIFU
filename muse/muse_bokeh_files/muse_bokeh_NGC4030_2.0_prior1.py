...
# directory path
path1 = '/Users/erickaguirre/Desktop/NaI_MCMC_output/NGC4030_2.0_test_prior1/'
# fits file path
...
tooltips3 = [("(x,y)", "($x{0}, $y{0})"), ("Dispersion Velocity (km/s)", "@image{0.000}")]
tooltips4 = [("(x,y)", "($x{0}, $y{0})"), ("E(B-V)", "@image{0.000}")]
...
p8_1 = figure(title="Samples logN prior1 range (11.4-16.0)", y_axis_label='lambda_red')
...
p11.x_range.range_padding = p10.y_range.range_padding = 0
...
# samples grid plot for each parameter in plot 8
...
# plot 4 MUSE cube Dust Attenuation Map
# must give a vector of image data for image parameter
# Plasma265 (purple to orange)
# Blues8 (blue to white)
color_mapper4 = LinearColorMapper(palette='Inferno256',low=0,high=0.75)
color_bar4 = ColorBar(color_mapper=color_mapper4, label_standoff=12)   
dust_map = p4.image(image=[np.transpose(ebv_map)], x=0, y=0, dw=ebv_map.shape[0],dh=ebv_map.shape[1], 
        color_mapper=color_mapper4)
p4.add_layout(color_bar4,'right')

# add hover tool 
p4.add_tools(HoverTool(tooltips = tooltips4, renderers = [dust_map]))
...
def update_data(attr):
    ...
    p8_1_update = figure(title="Samples logN prior1 range (11.4-16.0)", y_axis_label='lambda_red')
