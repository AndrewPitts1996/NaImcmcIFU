...
# directory path
path1 = '/Users/erickaguirre/Desktop/NaI_MCMC_output/NGC4030_2.0_test/'
# fits file path
...
color_mapper1 = LinearColorMapper(palette=cc.coolwarm,low=np.percentile(stellar_vel, 1),high=0.6*np.max(stellar_vel))
color_bar1 = ColorBar(color_mapper=color_mapper1, label_standoff=12)
stellar_velocity = p1.image(image=[stellar_vel], x=0, y=0, dw=img_data.shape[0],dh=img_data.shape[1], \
...
color_mapper3 = LinearColorMapper(palette='Viridis256',low=50,high=0.51*np.max(stellar_sigma))
color_bar3 = ColorBar(color_mapper=color_mapper3, label_standoff=12)                                  
stellar_dispersion = p3.image(image=[stellar_sigma], x=0, y=0, dw=stellar_sigma.shape[0],dh=stellar_sigma.shape[1],
...
narrow_band = p4.image(image=[img_data], x=0, y=0, dw=img_data.shape[0],dh=img_data.shape[1], 
...
gas_velocity = p5.image(image=[gas_vel_map], x=0, y=0, dw=img_data.shape[0],dh=img_data.shape[1],
...
layout_row2 = row(children=[p6,g,p1])
curdoc().add_root(layout_row2)
layout_row3 =row(children=[p2])
