...
# obtain H-alpha emission line flux from MUSE cube
...
# flux_transpose = np.transpose(NGC4030_cube0_6['FLUX'].data, axes=(0,2,1))
# model_transpose = np.transpose(NGC4030_cube0_6['MODEL'].data, axes=(0,2,1))
# ivar = np.transpose(NGC4030_cube0_6['IVAR'].data, axes=(0,2,1))
# error = np.sqrt(1/ivar)
...
# directory path
path1 = '/Users/erickaguirre/Desktop/NaI_MCMC_output/NGC4030_1.0_test/'
# fits file path
...
def corner(binid):
    binid_indx = np.where(table_uniq['bin']== binid)
    samples = table_uniq['samples'][binid_indx][0]
...
source8 = ColumnDataSource(data = dict(iterations = iterations, lambda_red = samples[:,0], \
                                       N = samples[:,1], b_d = samples[:,2], C_f = samples[:,3]))
...
p4 = figure(title='Na I Region Narrow-Band Image',tools=tools1, plot_width=535, plot_height=480,toolbar_location="below")
...
p8_1.step('iterations','lambda_red',source=source8,line_color='black')
p8_1.yaxis.axis_label = 'lambda_red'
p8_2.step('iterations','N',source=source8,line_color='black')
p8_2.yaxis.axis_label = 'N'
p8_3.step('iterations','b_d',source=source8,line_color='black')
p8_3.yaxis.axis_label = 'b_d'
p8_4.step('iterations','C_f',source=source8,line_color='black')
p8_4.yaxis.axis_label = 'C_f'
...
binid_label = Label(x=25,y=45, x_units='screen', y_units='screen', text = binid_text)
spaxel_label_p6 = Label(x=25,y=28, x_units='screen', y_units='screen', text = spaxel_text_p6)
...
gas_stell_velocity = p10.image(image=[np.transpose(gas_vel_map+stellar_vel)], x=0, y=0, dw=img_data.shape[0],dh=img_data.shape[1],
         color_mapper=color_mapper5)
...
def update_data(attr):
    ...
    samples = table_uniq['samples'][bin_indx][0]
    iterations = np.arange(samples.shape[0])
    source8.data = dict(iterations = iterations, lambda_red = samples[:,0], \
                                           N = samples[:,1], b_d = samples[:,2], C_f = samples[:,3])
...
layout_row2 = row(children=[p6,g,p9])
curdoc().add_root(layout_row2)
layout_row3 =row(children=[p2,p1,p10])
